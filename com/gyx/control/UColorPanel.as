package com.gyx.control
{
	...
	class UColorPanel extends Sprite
	{
		var col = 10;
		...
		var colorAlpha:UNumberAdjuster;
		var colorWell:UColorWell;
		var colorSelectRect:Sprite;
		var colorFocusRect:Sprite;
		var rowCount:int;
		var head:UGirdLayout, out:UGirdLayout;
		var colorFarm:Sprite;
		
		public function UColorPanel(selectedColor:uint = 0x000000, selectedColorAlpha:Number = 1, rowCount:int = 10)
		{
			
			this.selectedColor = selectedColor;
			this.selectedColorAlpha = selectedColorAlpha;
			
			head = new UGirdLayout(1, DefaultStyle.noPaddingGird);
			out = new UGirdLayout(2, DefaultStyle.girdCenter);
			...
			colorAlpha = new UNumberAdjuster({label: "透明度", value: Math.floor(selectedColorAlpha * 100)});
			...
			{
				colorFields[i] = new UColorField(colors[i]);
				colorFarm.addChild(colorFields[i]);
				colorFields[i].x = (i % rowCount) * (UColorField.w) + 2;
				colorFields[i].y = int(i / rowCount) * (UColorField.h) + 2;
			}
			...
			colorSelectRect.graphics.drawRect(-1, -1, UColorField.w + 2, UColorField.h + 2);
			colorSelectRect.graphics.lineStyle(1, 0x000000);
			colorSelectRect.graphics.drawRect(0, 0, UColorField.w, UColorField.h);
			colorFarm.addChild(colorSelectRect);
			
			colorFocusRect = new Sprite();
			colorFocusRect.graphics.lineStyle(1, 0xcceeff);
			colorFocusRect.graphics.drawRect(0, 0, UColorField.w, UColorField.h);
		...
		{
			if (e.target is UColorField && !e.buttonDown)
			{
		...
		function fieldMouseDown(e:MouseEvent)
		{
			if (e.target is UColorField)
			{
				selectField(e.target as UColorField)
			}
		}
		...
		function alphaChange(e:Event)
		{
			...
			colorWell.drawColor(selectedColor, selectedColorAlpha);
			dispatchEvent(new UColorPickerEvent(UColorPickerEvent.ALPHA_CHANGE));
		}
		...
		function selectField(colorField:UColorField)
		{
			selectedColor = colorField.color;
			colorTxt.text = formatColor(selectedColor);
			colorSelectRect.x = colorField.x;
			colorSelectRect.y = colorField.y;
			dispatchEvent(new UColorPickerEvent(UColorPickerEvent.COLOR_CHANGE));
		}
